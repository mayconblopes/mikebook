...
def index(request):
    ...
    final_suggentions_list = [x for x in list(new_suggestions_list) if (x not in list(current_user))]
    random.shuffle(final_suggentions_list)

    username_profile = []
    username_profile_list = []

    for user in final_suggentions_list:
        username_profile.append(user.id)

    for id in username_profile:
        profiles_list = Profile.objects.filter(id_user=id)
        username_profile_list.append(profiles_list)
    ...
    return render(request, 'index.html', {
        ...
        'posts': feed_list,
        'suggetions_username_profile_list': suggetions_username_profile_list[:4],
    })
    ...
    if request.method == 'POST':
        ...
        if password == password2:
            if User.objects.filter(email=email).exists():
                messages.info(request, 'Esse e-mail já foi utilizado')
                return redirect('signup')
            elif User.objects.filter(username=username).exists():
                messages.info(request, 'Esse nome de usuário já foi utilizado')
                return redirect('signup')
            else:
            ...
            messages.info(request, 'A senha não confere')
            return redirect('signup')
